-- Admin profiles may retain historical Owner rows, but cannot own arenas.
...
begin
    ...
    if exists (
        select 1
        from public.owners owner_profile
        join public.admins admin_profile on admin_profile.user_id = owner_profile.user_id
        where owner_profile.id = p_owner_id
    ) then
        raise exception 'An Admin account cannot own an arena';
    end if;
